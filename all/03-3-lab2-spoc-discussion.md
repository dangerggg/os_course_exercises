...
* 理解如何实现建立页表（主要自学和网上查找）
  ...
  https://blog.csdn.net/trochiluses/article/details/13001721
...
2. 不同特权级有什么区别？
   ...
   不同特权级能够访问的内存代码段和数据段等有区别，只有在Ring0才能够执行特权指令


3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

   CPL：当前进程的权限，是当前进程的自身属性

   DPL：段的权限，处于段描述符中

   RPL：进程以什么样的特权级来请求操作，位于段选择子中

   这三个属性控制了进程的权限关系，起到了保护作用


4. 写一个示例程序，完成4个特权级间的函数调用和数据访问时特权级控制的作用。

   满足关系即可，访问段max（cpl，rpl）<= dpl，访问门时CPL <= DPL[门] & CPL >= DPL[段]（从低优先级访问高优先级）

### 7.2 了解特权级切换过程

1. 一条指令在执行时会有哪些可能的特权级判断？

   访问数据段时，需要检查RPL，CPL和目标数据段的DPL之间的关系

   发生中断时，需要检查门描述符的DPL和CPL的关系，已经经过门之后访问的DPL和CPL的关系

2. 在什么情况下会出现特权级切换？

   产生中断，优先级较低的进程需要高优先级提供的服务

3. int指令在ring0和ring3的执行行为有什么不同？

   在ring0不需要切换堆栈，因此在Ring0也不需要将SS:ESP压栈


4. 如何利用int和iret指令完成不同特权级的切换？

   伪造一个中断的栈现场，并且修改SS和CS的RPL，CPL


5. TSS和Task Register的作用是什么？

   TSS为了保存当前优先级的栈位置和状态以及一系列寄存器内容，官方说法是保存任务切换的现场。

   Task Register是为了提高运行的效率，将原本位于内存中GDT表中的TSS基址存到寄存器中。
...
1. 一条指令执行时最多会出现多少次地址转换？
   ...
   两次，由逻辑地址经MMU查段表转换成线性地址，如果使能了页机制，线性地址需要经过转换变成物理地址。

2. 描述X86-32的MMU地址转换过程；

   通过段寄存器查找段描述符得到对应的段的基址，加上偏移之后便是线性地址，如果没有使能页机制，则得到的线性地址就是物理地址，如果使能了页机制，还需要通过分级查找页表来得到真正的物理地址。具体的过程在课件上都有详细说明，就不列出。

### 7.4 了解UCORE建立段/页表

> 本节内容脱离代码无法很好完成，准备阅读代码完成实验之后再作答。
